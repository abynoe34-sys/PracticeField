...
**HELD until the vocabulary lands in the catalogue (owner-directed):** the derived quantities (thumbs-together / palm-normal computation), the `landmark_derivations.py` vocabulary extensions + Thumb/Index repointing, the Layer-3 hand-need signal wiring, and the **~40-row `Partial`→`Needs Motion` retiering sweep** (use each row's `Measurable Signal` note to retier ONLY rows `Partial` *solely* for hand detail — not gaze/ball/defender). **Also still needed: two short test clips** for the derived-quantity verification — thumbs-diamond→pinkies-basket, and palms up/forward/out — which the current clips don't contain.
