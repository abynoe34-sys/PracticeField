...
**HELD until the vocabulary lands in the catalogue (owner-directed):** the derived quantities (thumbs-together / palm-normal computation), the `landmark_derivations.py` vocabulary extensions + Thumb/Index repointing, the Layer-3 hand-need signal wiring, and the **`Partial`→`Needs Motion` retiering sweep** (use each row's `Measurable Signal` note to retier ONLY rows `Partial` *solely* for hand detail — not gaze/ball/defender). **Verified candidate count = 18** (not the earlier unverified "~40"): 7 Catching→Extend&Bait + 7 Catching→The Strike + 4 Press-Release hand-fighting rows — treat 18 as a **floor** until a full 281-row `Measurable Signal`/`Pose Landmarks` scan runs (text-search matching may miss rows that need hands but don't yet say so). Rows correctly EXCLUDED (reference wrists for position/distance, not hand *shape*): Kickstep Crossover Plant Step-Hands, Diamond Release-The Diamond Cut-Hands, both Release "Hesitation"-Hands. **Also still needed: two short test clips** for the derived-quantity verification — thumbs-diamond→pinkies-basket, and palms up/forward/out — which the current clips don't contain.
