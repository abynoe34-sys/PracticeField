...
**⚠️ SOURCE OF TRUTH SUPERSEDED 2026-09-07 — read the top-of-file changelog entry first.** As of that date the official catalogue is the Supabase **`checkpoints_v2`** table (1,638 rows, rebuilt taxonomy), and **Airtable + `build_ruleset.py` are DEPRECATED (do not author new content there)**. Everything in THIS section describes the *legacy* Airtable→JSON pipeline. **Resolver-source status (updated 2026-09-08):** `layer3_resolver.py` now reads **QB, WR and TE from `checkpoints_v2`** (via `checkpoints_v2_source.py`), so for those three positions the JSON described here is **no longer read**; **DB/RB/OL still resolve from this JSON path** pending their own annotation + wiring. It is no longer where content should be edited for any position (`checkpoints_v2` is the source of truth). Legacy detail follows unchanged:
