...
**STILL HELD — the 18-row `Partial`→`Needs Motion` retiering sweep** (verified count 18, a floor; use each row's `Measurable Signal` note to retier ONLY rows `Partial` *solely* for hand detail — not gaze/ball/defender). Rows correctly EXCLUDED: Kickstep Crossover Plant Step-Hands, Diamond Release-The Diamond Cut-Hands, both Release "Hesitation"-Hands. **Blocker (owner):** the coaching shape NAMES don't yet map cleanly onto the filmed holds — clip 1's overhead+chest holds read as **pinky-together**, the low holds as **index-together**, which contradicts the spec's Thumbs-Diamond→Pinkies-Basket ordering. Retiering each row to a shape signal needs the owner to confirm which 5s hold = which named shape (or that the filmed order differs). The Strike rows whose note says "needs object detection" (the ball) will STAY `Partial` regardless — no ball tracking. Also still useful: a gloved clip (owner, later). Owner decision 3 (hand floor 0.6–0.7) stays pinned to 0.6 — impact-frame confidence looked healthy (min 0.54/0.62) but these are held shapes, not contested catches.
