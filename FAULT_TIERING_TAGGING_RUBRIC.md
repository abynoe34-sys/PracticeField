...
## 17. TE has-fault pass EXECUTED (2026-09-18) — TE fully tagged (5th complete position); WR→TE transfer + 4 checks

The 260 TE has-fault rows: **242 WR twins transferred** + **18 TE-original judged fresh**.
- **Twin transfer (242):** TE receiving content is a WR copy reworded "receiver"→"tight end". Matched TE→WR on fault text normalized for that substitution; confirmed every normalized WR fault maps to a SINGLE (tier, severity, is_safety) so the copy is unambiguous, then transferred all three fields. Twins by technique: Routes 105, Release 89, Catching 17, First Step 14, Stance 11, Blocking 3, Ball Carry 3. The transfer carried is_safety onto the queued archetypes automatically — and caught the WR-904 twin (Cuts-Stop stiff-leg) my hand-list had missed, i.e. the transfer was more complete than the manual queue.
- **TE-original (18) judged fresh** with §5a + the 4 checks. Tiers calibrated to the no-fault TE precedent (TE Blocking/First Step/Stance = **Fundamental**, unlike WR blocking-in-space=Developing, because TE blocking is a core job; Release/chip = Developing). Result: **Safety/Fund 1**, **Fund/Major 15**, **Dev/Major 2**.
- **Signal-(c) catch on a TE-original row — 1020** "Head Dropping. Dropping the chin to the chest **upon contact**… slide right off the defender" → is_safety=Fundamental. A run-block head-into-contact archetype that NO WR transfer would have reached; caught by the established-name/signal-(c) check exactly as intended (twin of OL 631).
- **Flagged borderline (kept Fund/Major, not safety): TE 1004** "Standing Tall… raises the pad level and makes the tight end an easy target for defenders to jam or blow off the line." Unlike RB 1123 (ball-carrier "target area" for an open-field HIT = signal b), 1004's "target" is for a *jam/blow-off at the LOS* (press + bull = rep-loss) and is pad-level language (ruled correct-technique, not safety). Surfaced for owner since it shares "standing tall"/"target" vocabulary with 1123.
- **TE safety = 8:** transferred 1237 Diving + 1238 Leading (Fund), 4 route-knee twins (Comeback 1433 / Curl 1445 / Hitch 1461 / Cuts-Stop 904-twin, Developing), pre-existing 1239 no-fault head-up (Fund) + TE-original 1020 (Fund).
- Verified live: TE 0 untagged, 0 no-fault rows carry severity (Option A), 0 safety rows carry severity. Catalogue **1391/1665**, safety 39. Snapshot regenerated (1391); resolver 238/238, cleaning 28/28.
