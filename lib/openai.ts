...
import { findCBDrillsForPainPoint } from './position-drills/cb'
import { findSSDrillsForPainPoint } from './position-drills/ss'
import { findFSDrillsForPainPoint } from './position-drills/fs'
...
export async function generateTrainingPlanAI(params: GeneratePlanParams): Promise<Exercise[]> {
  ...
  const DRILL_LIBRARY_POSITIONS = ['RB','WR','QB','OL','TE','DL','LB','OLB','CB','SS','FS','DB','ST','FB','NB']
  if (params.painPoints.length > 0 && posNorm && DRILL_LIBRARY_POSITIONS.includes(posNorm)) {
    const finder = posNorm === 'WR' ? findWRDrillsForPainPoint
      ...
      : posNorm === 'CB' ? findCBDrillsForPainPoint
      : posNorm === 'SS' ? findSSDrillsForPainPoint
      : posNorm === 'FS' ? findFSDrillsForPainPoint
...
const POSITION_CUES: Record<string, string> = {
  ...
  CB:  'Cornerbacks are the primary outside cover defenders — evaluate press and man-coverage mechanics first, run-support second. Evaluate: press alignment (correct inside/head-up/outside shade based on WR split and coverage call?), jam effectiveness (punch the breastplate with two hands inside the frame — not the arms or shoulders), backpedal mechanics (chin over toes, feet cutting the grass, butt leading — or upright and on heels?), cushion management (opening hips at the right moment — too early gifts the WR a free stem, too late loses the flip on a deep route), hip flip / turn and run (low elbow plane, lead foot 180°, flip-and-accelerate in one step — or adding a gather step before the turn?), phase awareness (in-phase hip to hip: look for ball; out-of-phase trailing: find the far shoulder and smack the wrist — never play the ball when trailing), bail mechanics (1 yard outside the WR, eyes keyed through to QB — not staring at the WR\'s release), stalk block defeat (fit inside shoulder, lockout, shed toward ball), crack recognition and call. Reference drills: W Drill, Mirror Drill, Turn and Run, Press Jam, Cushion Drill, Phase Drill, Bail Technique, Stalk Block Defeat, Crack Recognition.',
  SS:  'Strong Safeties are the box enforcers — evaluate run support and physicality first, underneath coverage second, blitz third. Evaluate: block-destruction approach (does the SS strike first with two hands inside the frame and shed before the ball carrier cuts — or absorbs the block and gets carried?), box form tackling (head-across-the-bow: correct shoulder, cheek to the ball, wrap and drive feet through the whistle — or diving at ankles / leading with the crown?), alley fit on outside runs (pressing the inside hip of the ball carrier and forcing the sideline — or running full-speed and lunging?), hook-curl zone drop (dropping at 45 degrees to the landmark — not straight back, which narrows the coverage window), TE and RB coverage in man (outside-hand jam on the release, redirect inside toward help, hip-to-hip through the stem — or giving a free vertical?), blitz execution from the box (flat path off the snap, no arc, first contact is the QB — or telegraphing by leaning before the snap?). Reference drills: Block Destruction Shed, Box Form Tackle, Alley Fit Tackle, Hook/Curl Drop, Box Blitz Path, TE Coverage Man.',
  FS:  'Free Safeties are the center fielders of the defense — evaluate QB read and deep-zone discipline first, pursuit angle and ball skills second. Evaluate: pre-snap depth and alignment (12-15 yards deep, correct shade for the coverage rotation — or creeping forward before the snap?), QB read and break timing (does the FS key through the QB\'s drop and eyes to determine which zone to protect — or keys on receivers and gets drawn forward on short passes?), play-action discipline (does the FS slow feet and hold depth on run-action before confirming pass — or bites on the fake and vacates the deep middle, opening the post or seam?), over-the-top rotation timing (does the FS arrive over the CB\'s receiver before the ball is thrown — or arrives a step late leaving the CB in man-island?), cross-field range (does the FS turn the hips fully to rotate and arrive at the far corner — or side-shuffles and arrives too late?), pursuit angle (running to the interception point, not toward the ball carrier — or flat pursuit that overruns the angle?), high-point catch mechanics (tracking the ball over the inside shoulder, leaping to the highest point — or reaching for the ball at waist height and getting undercut?). Reference drills: QB Read Drops, Center Field Zone Drop, Play-Action Discipline, Angle Pursuit, Cross-Field Range, High-Point Tracking.',
  DB:  'Generic defensive back evaluation (use when specific DB position is unspecified). Evaluate: pre-snap alignment and leverage (correct depth and shade for coverage call?), backpedal mechanics (chin over toes, feet cutting the grass, butt leading?), cushion management (opening hips too early or too late?), hip flip / man turn (low elbow plane — 90° or less), phase awareness (in-phase hip to hip: look for ball; out-of-phase trailing: play head and hands, smack far wrist), zone drop footwork (landmark depth, T-step or foot-fire on downhill breaks), run/pass read discipline (does the DB bite on play-action?), open-field tackling (shorten stride and sink hips on approach, press inside hip to flatten carrier\'s move), ball skills (tracking, highest-point catch, strip technique before tackle).',
  NB:  'NOTE: The Nickelback (NB) is the 5th defensive back deployed in nickel packages (3+ WR sets). The NB is a hybrid CB/LB — evaluate both coverage skills and run-support/tackling skills. Evaluate: slot alignment and shade (inside shade to take away the quick slant — not head-up allowing a free inside release), man coverage trail technique on slot WR (hip-to-hip inside shade — does the NB stay in contact through the route or give a free release?), hip flip in space (does the NB flip the hips sharply in tight quarters without an extra gather step?), mirror drill reactive agility (reading the slot WR\'s hips not the feet — gets juked or stays in phase?), press jam from inside shade (punch the outside shoulder to redirect the slot WR outside — or jamming straight and opening the slant window?), pick/rub/mesh route recognition (does the NB call "rub!" and switch immediately — or chase the assigned WR into the pick?), hook-curl and flat zone drops in nickel packages (correct 45-degree angle, correct landmark depth — or dropping straight back?), open-field tackling on slot WR after the catch (shorten stride, press inside hip — or lunging and missing?), blitz path execution (disguised stance until snap, flat gap path, speed-to-power — or telegraphing and looping wide?), screen recognition (does the NB diagnose OL releasing and flatten around the blocker to make the tackle — or gets sealed inside turning a 2-yard screen into 15 yards?), W Drill lateral quickness (pedal with purpose, low elbow on the break — or upright backpedal on heels?).',
  ...
  H:   'The holder\'s entire job is speed and consistency on every snap. Evaluate: snap-to-placement time (is the ball on the turf and set in under 0.30 seconds — or does the holder fumble-catch, then search for the spot?), lace rotation (are the laces rotating away from the kicker on every snap, including off-target snaps?), placement consistency (is the ball placed in exactly the same spot every rep — tilted correctly, not leaning left or right?), bad snap recovery (when the snap is high, low, or wide, does the holder aggressively field it and still get a kick off — or freeze?), kicker timing synchronisation (does the holder\'s placement timing match the kicker\'s approach rhythm — or does the kicker have to adjust their timing because the ball is early or late?), fake field goal execution (does the holder have the athletic ability and deception to run or throw on a fake — and can they avoid telegraphing the fake before the snap?).',
  PR:  'Evaluate: ball tracking (does the PR pick up the ball at the punter\'s foot and track it through the flight without losing it — or does the ball pop into the PR\'s face unexpectedly?), muff prevention (secure catch with two hands first, every time — is the PR at risk of muffing in sun, wind, or under pressure from a gunner?), fair-catch vs. return decision speed (does the PR signal fair catch early enough when a gunner is close — or wait too long and take a hit?), initial acceleration burst out of the catch (does the PR gain momentum immediately, or stop and look after the catch — allowing coverage to close?), vision and lane reading (does the PR keep their head up after the catch to find the crease in coverage, or tuck their chin and run into traffic?), elusiveness in open field (can the PR stop-start and make the first coverage player miss in space?), ball security (four-point carry maintained throughout the return — not one-arm carrying while being grabbed?).',
  KR:  'Evaluate: pre-kick positioning (is the KR set at the correct depth — typically 7 yards deep in the end zone — so they move forward to the ball rather than drifting backward?), ball-flight tracking and touchback judgment (does the KR read the ball early enough to make a clear in/out decision before it lands — or does the KR field kicks 5+ yards deep costing the offense field position?), catch security (both hands secure the ball immediately on the catch — no one-arm bobble that exposes the ball to a strip?), initial burst north-south (does the KR attack upfield through the seam before cutting — or cut too early and collapse the wedge?), vision through the wedge (does the KR track the blocking lane upfield and cut off the first clear seam — or run laterally looking for a perfect hole that never opens?), ball security in traffic (four-point carry locked in as coverage arrives at full speed — not carrying low with one arm near contact?), communication with the second returner on sky kicks (does the KR call for the ball clearly and avoid confusion?).',
