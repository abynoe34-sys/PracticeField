"""
Re-run MediaPipe on 0606_rep1_9s.png at confidence=0.5 (default).
Saves overlay to check_9s_default.png and prints M1 lean_from_vertical.
"""
...
DST        = r"C:\Users\abyno\PracticeField\scripts\check_9s_default.png"
...
def m1_slope(lms):
    ...
    horiz = (sx - hx) if (lms[0].x > hx) else (hx - sx)
...
print(f"Detected {n} persons  (conf=0.5)\n")
print(f"{'Person':>6}  {'slope_deg':>10}  {'lean_from_vert':>14}  {'higher':>10}")
print("-" * 48)

for p_idx, lms in enumerate(result.pose_landmarks or []):
    col   = PERSON_COLOURS[p_idx % len(PERSON_COLOURS)]
    draw_person(frame, lms, w, h, col, f"P{p_idx}")
    raw, lean, higher = m1_slope(lms)
    if raw is None:
        print(f"{p_idx:>6}  {'N/A':>10}  {'N/A':>14}  {'N/A':>10}")
    else:
        print(f"{p_idx:>6}  {raw:>10.2f}  {lean:>14.2f}  {higher:>10}")
...
cv2.putText(frame, "conf=0.5", (w-180, h-15),
